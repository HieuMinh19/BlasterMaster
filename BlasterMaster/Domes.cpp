...
void CDomes::Update(DWORD dt, vector<LPGAMEOBJECT>* coObjects)
{
	...
	if (coEvents.size() == 0)
	{
		...
		{
			//DebugOut(L"khong va cham STATE = 0 \n");
...
{
	DebugOut(L"HandleWithoutObstruction \n");
	//DERECTION 1->UP // 2-> LEFT // 3->DOWN // 4->RIGHT
	switch (direction) {
		case 1:
...
{
	DebugOut(L"SetStateAfterFirstCollision \n");
	if (nx < 0.0f)
	{
		if (vy > 0.0f)
		{
...
{
	//DebugOut(L"CollisionHandleWithBrick ** NX: %f ** NY: %f \n",nx,ny);
	//DERECTION 1->UP // 2-> LEFT // 3->DOWN // 4->RIGHT
	switch (direction) {
	case 1:
		DebugOut(L"CASE 1 \n");
		...
		{
			DebugOut(L"CASE 1.1111111111 \n");
			newState = DOMES_STATE_LEFT_RIGHT;
		}
		if (state == DOMES_STATE_RIGHT_UP && nx < 0.0f)
		{
			DebugOut(L"CASE 1.22222222222 \n");
			newState = DOMES_STATE_RIGHT_LEFT;
		}
		break;
	case 2:
		DebugOut(L"CASE 2 \n");
		if (state == DOMES_STATE_LEFT_LEFT && ny < 0.0f) newState = DOMES_STATE_LEFT_UP;
		if (state == DOMES_STATE_RIGHT_LEFT && ny > 0.0f) newState = DOMES_STATE_RIGHT_DOWN;
		break;
	case 3:
		DebugOut(L"CASE 3 \n");
		if (state == DOMES_STATE_LEFT_DOWN && nx < 0.0f)
		{
			DebugOut(L"CASE 3.1111111111 \n");
			newState = DOMES_STATE_LEFT_LEFT;
		}
		if (state == DOMES_STATE_RIGHT_DOWN && nx > 0.0f)
		{
			DebugOut(L"CASE 3.2222222222222 \n");
			newState = DOMES_STATE_RIGHT_RIGHT;
		}
		break;
	case 4:
		...
		{
			DebugOut(L"CASE 4.1111111111111111 \n");
			newState = DOMES_STATE_LEFT_DOWN;
		}
		if (state == DOMES_STATE_RIGHT_RIGHT && ny < 0.0f)
		{
			DebugOut(L"CASE 4.2222222222222 \n");
			newState = DOMES_STATE_RIGHT_UP;
		}
		break;
	}
